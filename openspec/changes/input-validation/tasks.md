...
- [ ] 1.1 Create/expand vitest coverage in `tests/` for valid `path` query input that should continue normal response flow.
- [ ] 1.2 Add a failing vitest case for invalid `path` values asserting `400` JSON response and no roast payload.
